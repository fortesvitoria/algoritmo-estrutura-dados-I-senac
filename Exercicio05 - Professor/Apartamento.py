...
class Apartamento:
    def __init__(self, id, numeroApt = "Sem número", torre = Torre(id=1, nome="Torre A", endereco="Endereço padrão"), numeroVaga = "Aguardando vaga"):
        self.id = id
    ...
    def __str__(self):
        return f"Id: {str(self.id)} - Número do Apartamento: {self.numeroApt} - Número da Vaga: {str(self.numeroVaga)} - Torre: {str(self.torre)}"
